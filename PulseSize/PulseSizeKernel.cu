    ...
    ((uint2)(inXY)(KERNEL_XY)))
{
    ...
    {
        if ((inNormalEnabled && inCompatibilityEnabled) ||
            (!inNormalEnabled && !inCompatibilityEnabled)) {
        ...
        if (inNormalEnabled) {
            ...
            if (inHasFrequencyKeyframes && inAccumulatedPhase > 0.0f) {
                ...
                effectivePhase = inPhase + (inCurrentTime * inFrequency);
            }

            if (inWaveType == 0) {  
                m = sin(effectivePhase * 3.14159f);
            }
            else {  
                m = TriangleWaveDevice(effectivePhase / 2.0f);
            }

            float range = inGrow - inShrink;
            ds = (range * ((m + 1.0f) / 2.0f)) + inShrink;
        }
        else {
            if (inCompatWaveType == 0) {  
                m = sin(((inCurrentTime * inCompatFrequency) + inCompatPhase) * 3.14159f);
            }
            else {  
                m = TriangleWaveDevice(((inCurrentTime * inCompatFrequency) + inCompatPhase) / 2.0f);
            }

            float range = inCompatGrow - inCompatShrink;
            ds = (range * ((m + 1.0f) / 2.0f)) + inCompatShrink;
        }
    ...
    int hasFrequencyKeyframes)
{
    ...
    PulseSizeKernel << < gridDim, blockDim, 0 >> > ((float4 const*)src, (float4*)dst, srcPitch, dstPitch, is16f, width, height,
        ...
        normalEnabled, compatibilityEnabled, compatFrequency, compatShrink, compatGrow, compatPhase,
        compatWaveType, accumulatedPhase, hasFrequencyKeyframes);
