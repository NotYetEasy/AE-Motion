    ...
    ((uint2)(inXY)(KERNEL_XY)))
{
    ...
    {
        // Skip processing if strength is too small
        ...
        // Calculate sampling direction based on angle
        float angle_rad = -inAngle * 0.017453292519943295769236907684886; // PI/180
        float velocity_x = cos(angle_rad) * inStrength;
        float velocity_y = -sin(angle_rad) * inStrength;

        // Adjust velocity_x to account for non-square pixels
        ...
        // Limit number of samples based on blur strength
        ...
        // Get the original pixel (will be included in the accumulation)
        float4 origPixel = ReadFloat4(inSrc, inXY.y * inSrcPitch + inXY.x, !!in16f);

        // Initialize accumulators with the original pixel
        float accumR = origPixel.x;
        float accumG = origPixel.y;
        float accumB = origPixel.z;
        float accumA = origPixel.w;

        // Track the number of valid samples
        float totalWeight = 1.0f; // Start with 1 for the original pixel

        // Calculate inverse values outside the loop
        float inv_nSamples_minus_1 = 1.0f / (float)(nSamples - 1);

        // Accumulate samples - starting from 1 since we already included the original pixel
        for (int i = 1; i < nSamples; i++)
        {
            ...
            // Check if sample is inside texture bounds (match the XML shader behavior)
            if (sample_norm_x >= 0.0f && sample_norm_x <= 1.0f &&
                ...
                // Only read pixels that are within the image boundaries
                ...
                // Increase the weight counter for valid samples
                totalWeight += 1.0f;
            }
        }

        // Calculate the final weight based on the number of valid samples
        float finalWeight = (totalWeight > 0.0f) ? (1.0f / totalWeight) : 1.0f;

        // Average the accumulated samples using the actual number of valid samples
        float4 result;
