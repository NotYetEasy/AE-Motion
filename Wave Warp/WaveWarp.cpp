    ...
    PixelType* outP)
{
    ...
    if (wiP) {
        PF_FpLong x_norm = xL / PF_FpLong(wiP->width);
        PF_FpLong y_norm = yL / PF_FpLong(wiP->height);

        PF_FpLong direction_rad = wiP->direction * 0.0174533;
        PF_FpLong warpangle_rad = wiP->offset * 0.0174533;

        PF_FpLong a1 = direction_rad;
        PF_FpLong a2 = direction_rad - warpangle_rad;

        PF_FpLong st_x = x_norm;
        PF_FpLong st_y = 1.0 - y_norm;
        ...
        PF_FpLong finalSrcX = x_norm + offs_x;
        PF_FpLong finalSrcY = y_norm - offs_y;

        bool outsideBounds = false;

        if (wiP->xTiles) {
            if (wiP->mirror) {
                PF_FpLong fracPart = fmod(fabs(finalSrcX), 1.0);
                int isOdd = (int)floor(fabs(finalSrcX)) & 1;
                finalSrcX = isOdd ? 1.0 - fracPart : fracPart;
            }
            else {
                finalSrcX = finalSrcX - floor(finalSrcX);
                if (finalSrcX < 0.0) finalSrcX += 1.0;
            }
        }
        else {
            if (finalSrcX < 0.0 || finalSrcX >= 1.0) {
        ...
        if (wiP->yTiles) {
            if (wiP->mirror) {
                PF_FpLong fracPart = fmod(fabs(finalSrcY), 1.0);
                int isOdd = (int)floor(fabs(finalSrcY)) & 1;
                finalSrcY = isOdd ? 1.0 - fracPart : fracPart;
            }
            else {
                finalSrcY = finalSrcY - floor(finalSrcY);
                if (finalSrcY < 0.0) finalSrcY += 1.0;
            }
        }
        else {
            if (finalSrcY < 0.0 || finalSrcY >= 1.0) {
        ...
        if (outsideBounds) {
            *outP = PixelType();     
            ...
            finalSrcX = WW_MAX(0.0, WW_MIN(0.9999, finalSrcX));
            finalSrcY = WW_MAX(0.0, WW_MIN(0.9999, finalSrcY));

            PF_FpLong sample_x_f = finalSrcX * wiP->width;
            PF_FpLong sample_y_f = finalSrcY * wiP->height;

            *outP = SampleBilinear<PixelType>((PixelType*)(wiP->srcData), sample_x_f, sample_y_f,
                wiP->width, wiP->height, wiP->rowbytes, wiP->xTiles, wiP->yTiles, wiP->mirror);
        }
    }
    else {
        *outP = *inP;
    ...
    PF_PreRenderExtra* extraP)
{
    ...
    if (infoP) {
        ...
        infoP->direction = ((float)-cur_param.u.ad.value / 65536.0f);
