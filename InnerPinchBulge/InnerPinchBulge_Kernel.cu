#ifndef InnerPinchBulge
#define InnerPinchBulge
        ...
        if (dist < inRadius) {
            ...
            if (inStrength > 0.0f) {
                if (inUseGaussian) {
                    float factor = 1.0f;
                    if (p > 0.0f) {
                        float safeDist = fmaxf_custom(dist, 0.001f);
                        float gp = fminf_custom(fmaxf_custom((p - 0.0f) / (inRadius / safeDist - 0.0f), 0.0f), 1.0f);
                        gp -= 1.0f;
                    ...
                    float safeDist = fmaxf_custom(dist, 0.001f);
                    float t = fmaxf_custom(0.0f, fminf_custom((p - 0.0f) / (inRadius / safeDist - 0.0f), 1.0f));
                ...
                float safeDist = fmaxf_custom(dist, 0.001f);
                float factor = mixf(1.0f, pow(p, 1.0f + inStrength * 0.75f) * inRadius / safeDist, 1.0f - p);
                offsetX *= factor;
                offsetY *= factor;
            }
