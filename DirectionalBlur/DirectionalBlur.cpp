...
static inline void SampleBilinear(
    ...
    PixelType* outP)
{
    if (!src_data || !outP || rowbytes <= 0 || width <= 0 || height <= 0) {
        if (outP) {
    ...
    if constexpr (std::is_same_v<PixelType, PF_PixelFloat>) {
        ...
        const A_long stride = (rowbytes > 0) ? (rowbytes / sizeof(PF_PixelFloat)) : 0;
        ...
        const A_long stride = (rowbytes > 0) ? (rowbytes / sizeof(PF_Pixel16)) : 0;
        ...
        const A_long stride = (rowbytes > 0) ? (rowbytes / sizeof(PF_Pixel8)) : 0;
    ...
    PixelType* outP)
{
    ...
    if (!biP->src || biP->width <= 0 || biP->height <= 0 || biP->rowbytes <= 0) {
        *outP = *inP;
        return PF_Err_BAD_CALLBACK_PARAM;
    }
    ...
    const float inv_nSamples_minus_1 = (nSamples > 1) ? (1.0f / static_cast<float>(nSamples - 1)) : 1.0f;
    ...
    BlurInfo* infoP)
{
    ...
    if (!err && extraP->input->what_gpu == PF_GPU_Framework_OPENCL)
    {
        ...
        if (!cl_gpu_dataP || !cl_gpu_dataP->directional_blur_kernel) {
            return PF_Err_INTERNAL_STRUCT_DAMAGED;
        }
    ...
    else if (!err && extraP->input->what_gpu == PF_GPU_Framework_DIRECTX)
    {
        ...
        if (!dx_gpu_data || !dx_gpu_data->mContext || !dx_gpu_data->mDirectionalBlurShader) {
            return PF_Err_INTERNAL_STRUCT_DAMAGED;
        }
        ...
        bool dx_success = true;
        dx_success &= shaderExecution.SetParamBuffer(&blur_params, sizeof(DirectionalBlurParams));
        dx_success &= shaderExecution.SetUnorderedAccessView(
            (ID3D12Resource*)dst_mem,
            blur_params.mHeight * dst_row_bytes);
        dx_success &= shaderExecution.SetShaderResourceView(
            (ID3D12Resource*)src_mem,
            blur_params.mHeight * src_row_bytes);
        dx_success &= shaderExecution.Execute((UINT)DivideRoundUp(blur_params.mWidth, 16), (UINT)DivideRoundUp(blur_params.mHeight, 16));
    ...
    else if (!err && extraP->input->what_gpu == PF_GPU_Framework_METAL)
    {
        ...
        if (!metal_dataP || !metal_dataP->directional_blur_pipeline) {
            return PF_Err_INTERNAL_STRUCT_DAMAGED;
        }
        ...
        NSError* metal_error = [commandBuffer error];
        if (metal_error) {
            err = PF_Err_INTERNAL_STRUCT_DAMAGED;
        }
    }
    ...
    bool isGPU)
{
    ...
    if (extraP->input->pre_render_data) {
        infoP = reinterpret_cast<BlurInfo*>(extraP->input->pre_render_data);
    }
    else {
        infoP = (BlurInfo*)malloc(sizeof(BlurInfo));
        if (infoP) {
            memset(infoP, 0, sizeof(BlurInfo));
            infoP->strength = DBLUR_STRENGTH_DFLT;
            infoP->angle = 0.0f;
        }
    }
    ...
    if (infoP) {
        ...
        if (err || !output_worldP) {
            if (input_worldP) {
                extraP->cb->checkin_layer_pixels(in_data->effect_ref, DBLUR_INPUT);
            }
            if (infoP && !extraP->input->pre_render_data) {
                free(infoP);
            }
