...
// CUDA kernel; see InnerPinchBulge.cu
	...
	PF_GPUDeviceSetupExtra* extraP)
{
	...
	if (extraP->input->what_gpu == PF_GPU_Framework_CUDA) {
		...
		// Define helper functions for smoothstep and mix
	...
	PixelType* outP)
{
	...
	if (piP) {
		...
		float centerX = 0.5f + (PF_FpLong)piP->center_x / 10000.0f;
		float centerY = 0.5f - (PF_FpLong)piP->center_y / 10000.0f;
		...
		// Store original offsets for feathering
		...
		float finalSrcX = offsetX + centerX;
		float finalSrcY = offsetY + centerY;

		// Sample the input at the calculated coordinates with tiling parameters
	...
	PinchBulgeInfo* infoP)
{
	...
	// Determine pixel depth and call appropriate iteration suite
	double bytesPerPixel = (double)input_worldP->rowbytes / (double)input_worldP->width;

	if (bytesPerPixel >= 16.0) { // 32-bit float
	...
	else if (bytesPerPixel >= 8.0) { // 16-bit
	...
	PinchBulgeInfo* infoP)
{
	...
	else if (!err && extraP->input->what_gpu == PF_GPU_Framework_OPENCL)
	{
		...
		// Set the arguments for InnerPinchBulge kernel
	...
	PF_LayerDef* output)
{
	...
	// FIXED: Use u.bd.value for boolean parameters
