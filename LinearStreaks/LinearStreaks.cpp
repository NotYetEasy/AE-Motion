	...
	PF_PreRenderExtra* extraP)
{
	...
	if (infoP) {
		...
		float strength = infoP->strength;
		float angle = infoP->angle;
		float rad = angle * 0.0174533f;     

		float velocityX = cos(rad) * strength;
		float velocityY = -sin(rad) * strength;

		PF_Fixed aspect_ratio = in_dataP->pixel_aspect_ratio.num / (float)in_dataP->pixel_aspect_ratio.den;
		velocityX *= aspect_ratio;

		int max_disp_x = (int)ceil(fabs(velocityX) * in_dataP->width);
		int max_disp_y = (int)ceil(fabs(velocityY) * in_dataP->height);

		req.preserve_rgb_of_zero_alpha = TRUE;        

		req.rect.left -= max_disp_x;
		req.rect.right += max_disp_x;
		req.rect.top -= max_disp_y;
		req.rect.bottom += max_disp_y;
