    ...
    ((uint2)(inXY)(KERNEL_XY)))
{
    if (inXY.x < inWidth && inXY.y < inHeight)
    {
        ...
        float centerX = width / 2.0f + inPositionX;
        float centerY = height / 2.0f - inPositionY;

        float centerNormX = centerX / width;
        float centerNormY = centerY / height;

        st_x += (centerNormX - 0.5f) * -1.0f;
        st_y += (centerNormY - 0.5f) * -1.0f;
        ...
        float sample_x = adjusted_x + ((dx - 0.5f) * inMagnitude * width);
        float sample_y = adjusted_y - ((dy - 0.5f) * inMagnitude * height);      
        ...
        if (inXTiles) {
            if (inMirror) {
                float fracPart = fmodf(abs(norm_x), 1.0f);
                int isOdd = (int)floorf(abs(norm_x)) & 1;
        ...
        if (inYTiles) {
            if (inMirror) {
                float fracPart = fmodf(abs(norm_y), 1.0f);
                int isOdd = (int)floorf(abs(norm_y)) & 1;
